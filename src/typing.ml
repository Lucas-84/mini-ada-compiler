...
and type_expr env (e, loc) = match e with
  ...
  | Eaccess (Aident i) ->
    if dtyp_of_ident i loc env = Dtyp_fun_def then type_expr env (Ecall (i, []), loc)
...
and type_decl env (d, loc) = match d with
  ...
  | Dfunction (i, pl, rt, dl, sl) ->
    ...
    if rt <> Tunit && not is_return then raise (No_return (i, loc));
...
and type_stmt env (s, loc) = match s with
  ...
  | Sfor (i, rev, e1, e2, sl) ->
    ...
    let (tsl, is_return) = type_stmt_list env' sl in
    TSfor (i, rev, et1, et2, tsl), is_return
